...
const SUPPORTED = ["111", "115", "130"] as const;
...
async function compute(modelo: Modelo, admin: ReturnType<typeof createSupabaseAdmin>, empresaId: string, ejercicio: number, periodo: Trimestre) {
  if (modelo === "111") {
...
export async function GET(request: NextRequest, ctx: { params: Promise<{ modelo: string }> }) {
  ...
  const periodo = (parsed.data.periodo ?? trimestre) as Trimestre;
