...
export default async function SelfServeHomePage() {
  ...
  if ((existingCompanies ?? []).length === 0 && auth.user.user_metadata?.business_name) {
    await supabase.from("empresas").insert({
      ...
      nombre: String(auth.user.user_metadata.business_name),
  ...
  const { data: companies } = await supabase
    .from("empresas")
    .select("id,nombre,nif,cliente_slug,account_type,onboarding_source,estado")
    .eq("owner_user_id", auth.user.id)
    .limit(10);
          ...
          <table className="table">
            ...
            <tbody>
              {(companies ?? []).length === 0 ? (
                ...
                <tr key={company.id}>
                  <td>{company.nombre || company.cliente_slug || company.id.slice(0, 8)}</td>
                  <td>{company.nif}</td>
